...
export const clearFirestoreCache = async () => {
  try {await clearIndexedDbPersistence(db);} catch (error) {:', error.message);
  }
};
...
export const testFirebaseConnection = async () => {
  try {// Log Firebase project details (safe to log, no sensitive data)// Test Firestore connection
    const testDoc = doc(db, 'test-connection', 'test');

    await setDoc(testDoc, { timestamp: new Date().toISOString() });// Test Storage connection - try to get a reference
    const testStorageRef = storageRef(storage, 'test-connection/test.txt');// Test Authentication
    const user = auth.currentUser;

    if (user) {} else {}} catch (error) {
      // Error handling
    }
};
