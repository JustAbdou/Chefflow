...
export default function CoolingScreen({ navigation }) {
  ...
  // Fetch logs from cooling collection with date filter
  const fetchLogs = async () => {
    if (!restaurantId) return;

    try {
      console.log('🔍 Fetching cooling logs for restaurant:', restaurantId);
      ...
      const coolingCollection = getRestaurantCollection(restaurantId, 'cooling');
      const q = query(
        coolingCollection,
      ...
      console.log('📋 Fetched cooling logs:', allLogs.length);
      setLogs(allLogs);
    } catch (error) {
      console.error('❌ Error fetching cooling logs:', error);
      Alert.alert('Error', 'Failed to load cooling logs');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleSaveLog = async () => {
    ...
    if (!startTemp.trim()) {
      Alert.alert('Error', 'Please enter a start temperature');
      return;
    }
    if (!coolingTime.trim()) {
      Alert.alert('Error', 'Please enter cooling time');
      return;
    }
    if (!endTemp.trim()) {
      Alert.alert('Error', 'Please enter an end temperature');
      return;
    }
    ...
    try {
      ...
      await addDoc(coolingCollection, {
        item: foodItem.trim(),
      ...
      console.log('✅ Cooling log saved successfully');
      ...
      Alert.alert('Error', 'Failed to save cooling log');
    }
  };
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.centered}>
          ...
          <Text style={styles.loadingText}>Loading cooling logs...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      >
        ...
        <View style={styles.addLogSection}>
          <TouchableOpacity style={styles.addLogCard} onPress={() => setShowAddModal(true)}>
            ...
            <Text style={styles.addLogText}>Add Cooling Log</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.logsSection}>
          ...
          {selectedDateLogs.length === 0 ? (
            <View style={styles.emptyState}>
              <Ionicons name="snow-outline" size={48} color={Colors.textSecondary} />
              <Text style={styles.emptyText}>No cooling logs recorded for this date</Text>
              <Text style={styles.emptySubtext}>
                Tap "Add Cooling Log" to record temperature data
              </Text>
            </View>
          ) : (
          ...
          <View style={styles.overlay}>
            ...
            <View style={styles.modal}>
              ...
              <View style={styles.form}>
                ...
                <Text style={[styles.label, { marginTop: Spacing.lg }]}>Start Temperature (°C)</Text>
                <TextInput
                  style={styles.input}
                  value={startTemp}
                  onChangeText={setStartTemp}
                  placeholder="Enter start temperature"
                  placeholderTextColor={Colors.gray200}
                  keyboardType="numeric"
                />
                
                <Text style={[styles.label, { marginTop: Spacing.lg }]}>Cooling Time</Text>
                <TextInput
                  style={styles.input}
                  value={coolingTime}
                ...
                <Text style={[styles.label, { marginTop: Spacing.lg }]}>End Temperature (°C)</Text>
                <TextInput
                  style={styles.input}
                  value={endTemp}
